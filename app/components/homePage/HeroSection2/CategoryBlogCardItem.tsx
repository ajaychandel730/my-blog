...
const CategoryBlogCardItem = async ({ _id, title, date, banner }: Props) => {
  return (
    <Link href={`/blog/${_id}`} className="flex items-center border-t hover:bg-gray-200 dark:hover:bg-midnight-800 border-gray-200 dark:border-gray-800  p-2">
      <div className="w-full flex space-x-2">
        <div className="flex flex-1 flex-col space-y-1">
            <h3 className="font-semibold text-sm line-clamp-4 hover:underline">{title}</h3>
          <p className="text-tiny uppercase font-semibold text-gray-600">{date}</p>
        </div>
