...
const SignupSuccessModal = ({state}:Props) => {
 ...
 const stateHandler = (()=>{
  try{
    if(!state || typeof state !== "object" || !("message" in state) || !state.message){
      return;
     }
     ...
     if (
       ...
       onOpen();
       state.message = "";
     }else if( "error" in state && state.error !== null  && typeof state.error == "object"){
       if("server" in state.error && state.error.server == true){
         toast.error(String(state?.message) || "Something went wrong. Please try later.");
